...
| meditate ["topic"] [--quick] [--random-model \| --model-per-branch \| --ensemble] | `/crux-meditate` | Deep research exploration via a recursive tree of memory-informed subagents. Two orthogonal axes: **recursion mode** (`Research` default, depth-first serial recursion with mandatory inline citations and branch peer review; `--quick` for legacy parallel fan-out with warn-only citations and no peer review) and **model strategy** (`single` default, on the caller's model; `--random-model` single tree on a randomly-picked entry from `cruxMemories.meditate.modelPool`; `--model-per-branch` single tree with each top-level facet branch assigned a distinct pool model and descendants inheriting; `--ensemble` Ensemble Max running N parallel trees across the entire pool + cross-model aggregator). `--random-model`, `--model-per-branch`, and `--ensemble` are mutually exclusive. A merged pre-spawn gate covers cost, scope, and richness level selection (`compact` / `default` / `detailed` / `exhaustive`, `default` preselected) and offers in-place strategy swaps. Every meditation produces a paired HTML + PDF report with infographics, a citations index, and a mandatory adversarial review pass under `meditations/{yyyymmdd}-{topic-slug}/` (Ensemble Max additionally produces a `cross-model-synthesis.md` and ensemble report pair under `…-ensemble/`). A post-consolidation finalisation-enhancement gate (K10, multi-select 0–5) refines the report before adversarial review runs; skipping all reproduces previous behaviour. |
...
### Forget Workflow (executed by CRUX engine)

The forget command allows users to remove incorrect, outdated, or unwanted memories from the corpus. It is destructive and always requires explicit confirmation.

1. **Resolve input:** Accept one or more memory identifiers — short hash IDs (7-char hex), slugs, file paths, or a quoted search query. If no arguments are given, load the full memory index and present all memories for selection
2. **Display for confirmation:** Show each matched memory's ID, title, type, strength, and source. Always ask the user to confirm before proceeding — this operation is irreversible
3. **Delete memories:** For each confirmed memory, delete via `crux-skill-memory-crud`:
   - Remove the memory file (`*.memory.md` or `*.memory.crux.md`)
   - Remove the corresponding reference tracker (`{trackingDir}/{slug}.refs.yml`) if it exists
4. **Rebuild index:** Run `crux-skill-memory-index` to regenerate `.crux/memory-index.yml`
5. **Report:** Summarise what was deleted — count, types, and IDs of removed memories
...
- When the current chat session has explicitly enabled `/crux-amnesia`, that session override takes precedence over `enableMemories: "true"` for ordinary work:
...
- **Dev:** Run `/crux-forget` with a non-existent ID, verify graceful error message
...
`/crux-meditate` is the deep-research command. It is parameterised on two orthogonal axes — **recursion mode** (Research default, or `--quick`) and **model strategy** (`single` default, `--random-model`, `--model-per-branch`, or `--ensemble`) — and every meditation produces a paired HTML + PDF report with infographics, an in-page table of contents, and a citations index. Before any subagents spawn, four mandatory user gates fire: `Q-Depth-Selection`, `Q-Cost-and-Richness-Acknowledgment` (Sub-Q1 richness + Sub-Q2 proceed/mode-swap/strategy-swap/cancel), the Theme Preflight question sequence (Q1–Q5), and the facet-confirmation Pattern-B flow (`Q-Confirm-1` and `Q-Confirm-2`). All outputs live under `meditations/{yyyymmdd}-{topic-slug}/` (Ensemble Max produces an additional ensemble root with `cross-model-synthesis.md` and ensemble report pair).
...
- **User:** Verify `Q-Cost-and-Richness-Acknowledgment` fires immediately after depth selection with mode-aware totals (`~45 agents at depth 3 in Research`, `~42 in Quick`), and offers recursion-mode swaps (`switch_to_quick` / `switch_to_research`), model-strategy swaps (`switch_to_single` / `switch_to_random_model` / `switch_to_model_per_branch` / `switch_to_ensemble`, gated on `modelPool` size — `random` requires ≥1, `per_branch` requires ≥1, `ensemble` requires ≥2; the currently-active strategy is omitted), plus `cancel`
