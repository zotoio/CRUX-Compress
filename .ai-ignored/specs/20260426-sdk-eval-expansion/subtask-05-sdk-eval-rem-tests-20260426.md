...
- [x] Create `evals/sdk/tests/c-rem.test.ts`
- [x] C1 tests: REM Sleep interactive recommendations
- [x] C2 tests: REM Sleep `--yolo` mode auto-apply
- [x] C3 tests: REM Sleep conflict resolution
- [x] All tests use isolated worktrees with seeded memory fixtures
- [ ] All tests pass *(cannot verify — requires API key and ~30min runtime)*

## Definition of Done
- [x] `c-rem.test.ts` has at least 8 tests across C1, C2, C3
- [ ] All tests pass when run via `pnpm test:rem` *(cannot verify — requires API key and ~30min runtime)*
- [x] No linter errors
...
- Completed: 2026-04-26T12:28:00+10:00

### Work Log
- Read harness.ts for exact fixture/assertion function signatures
- Read j-recall.test.ts and p-amnesia.test.ts for established patterns
- Created c-rem.test.ts with 8 tests across C1 (3), C2 (3), C3 (2)
- C1 tests share the outer workspace; C2 and C3 each get their own isolated workspace for clean ground-truth assertions
- TypeScript compilation: only pre-existing Agent type errors (same as all other test files); no new errors introduced
- Linter: no errors
- "test:rem" vitest grep pattern (`'REM'`) confirmed to match describe block `"C: REM Sleep"`

### Design Decisions
- C2 and C3 each use a **separate isolated workspace** (created in nested `beforeAll`) so yolo-mode file mutations don't pollute other test groups
- C1 uses the shared outer workspace since it only reads/reports without file mutations
- Each test creates its own agent instance for isolation (consistent with j-recall pattern)
- Orphan tracker assertions use OR-pattern: filesystem ground truth OR output language match (agent may not have filesystem tools available in all configurations)
- Conflict tests verify both memory files still exist on disk after run (strongest assertion)

### Blockers Encountered
None

### Files Modified
- `evals/sdk/tests/c-rem.test.ts` (created)
- `specs/20260426-sdk-eval-expansion/subtask-05-sdk-eval-rem-tests-20260426.md` (checklist + notes)
