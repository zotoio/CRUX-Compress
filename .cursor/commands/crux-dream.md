...
- **Plan name** (e.g. `20260403-crux-memories`): The manager runs the dream extraction workflow on the completed plan. It verifies execution, analyses artifacts, compares with existing memories, detects conflicts, presents ranked candidates, and creates accepted memories. Pass `$ARGUMENTS` to the subagent as the plan name.
...
#### Dream Extraction (plan name)

1. Verifies the plan completed successfully (checks `_execution-state.yml`)
2. Analyses repository changes since plan start for scope assessment
3. Reads all plan artifacts (subtask files, execution reports, work logs, diffs)
...
8. Writes a dream summary to the plan directory
9. Rebuilds the memory index
10. Offers to archive the completed plan directory
...
5. Presents a structured report for user approval
6. Applies confirmed changes
7. Writes a REM summary
8. Rebuilds the memory index
