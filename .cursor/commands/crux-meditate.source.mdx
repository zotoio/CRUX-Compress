...
7. **Spawn N model-specific meditation trees in parallel**: For each entry `{slug, label}` in `modelPool`:
   - Create subdirectory `{ensembleWorkingDir}/model-{label-slug}/` (where `label-slug` is the kebab-case version of the label, e.g. `model-gpt-5.6-sol`, `model-opus-5`, `model-gemini-pro-3.1`).
...
- `crux-memory-dream`, `crux-memory-rem`, `crux-memory-recall`, `crux-memory-remember`, `crux-memory-forget` — mode-scoped memory agents for Dream / REM / Recall / Remember / Forget (non-Meditate workflows; prefer these over the deprecated umbrella dispatcher)
