...
When this command is invoked, spawn a `crux-memory-recall` subagent to query and display memories. Recall is **read-only** — it never modifies memory files on disk.

**User input escalation — CRITICAL**: This command uses **Pattern B (work first, then escalate)**. The subagent NEVER calls `AskQuestion` directly — the parent agent handles every post-display prompt (delete, consolidate, promote, skip) and resumes the subagent with the answers. For the full Pattern A / Pattern B contract and the `needs_user_input` YAML schema, see `.cursor/skills/_memory-shared.md#user-input-escalation` and `AGENTS.md`.

### Argument Handling

- **No arguments**: The subagent loads the memory index (`.crux/memory-index.yml`) and surfaces memories most likely to be relevant to the current conversation context. For each memory, it shows title, type, strength, reference count, and a brief rationale for why it was surfaced.
- **Quoted text** (e.g. `"performance optimization"`): The subagent searches existing memories by title, description, tags, and body content. Results are ranked by relevance with decompressed body content shown for compressed memories. Pass `$ARGUMENTS` to the subagent as the search query.
- **Spec name(s)** (e.g. `20260403-crux-memories`): The subagent finds all memories whose `source` field matches the given spec slug(s). Results are grouped by type. Pass `$ARGUMENTS` to the subagent as the spec name(s).
- **File path(s)** (e.g. `memories/learning/foo.memory.md`): The subagent reads the specified memory file(s) directly. Compressed files (`.memory.crux.md`) are decompressed for display. Full frontmatter and body are shown. Pass `$ARGUMENTS` to the subagent as the file path(s).
- **`--total`**: The subagent gathers the complete memory corpus — index metadata, all memory files, and their relationships — then generates an interactive canvas visualization of the entire memory system using only `cursor/canvas` SDK components and inline SVG. The `crux-memory-recall` agent owns the structural template loaded on this cold path. No table or text output is produced; the canvas is the deliverable.

### What Happens

1. The subagent reads `.crux/crux-memories.json` to load configuration
2. Based on the invocation mode, it either:
   ...
   - (`--total`) Reads the full memory index and all memory files, builds a relationship graph, and generates a canvas visualization using `cursor/canvas` SDK primitives and inline SVG (the `crux-memory-recall` agent owns the structural template for this cold path)
...
1. **Delete**: Present a follow-up `AskQuestion` listing each memory by its `id` and title as a multi-select. Spawn a `crux-memory-forget` subagent with the confirmed list to remove the memories and their trackers, then rebuild the index via `crux-skill-memory-index`.
2. **Consolidate**: Spawn a `crux-memory-rem` subagent scoped to the displayed memories. The subagent identifies near-duplicates, merges them, and compresses if `enableMemoryCompression` is active. Present the consolidation report to the user.
