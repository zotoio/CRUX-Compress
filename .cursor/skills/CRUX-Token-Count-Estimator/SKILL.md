---
name: CRUX-Token-Count-Estimator
description: Estimate token counts for markdown and code files using deterministic heuristics. Use when estimating tokens for CRUX compression, comparing file sizes, or analyzing token usage.
---

# CRUX Token Count Estimator
...
```bash
bash .cursor/skills/CRUX-Token-Count-Estimator/scripts/count-tokens.sh <file_path>
```
...
**Single file:**
```bash
bash .cursor/skills/CRUX-Token-Count-Estimator/scripts/count-tokens.sh CRUX.md
```

**Compare before/after compression:**
```bash
# Get source token count
bash .cursor/skills/CRUX-Token-Count-Estimator/scripts/count-tokens.sh rules/my-rule.mdc

# Get CRUX token count
bash .cursor/skills/CRUX-Token-Count-Estimator/scripts/count-tokens.sh rules/my-rule.crux.mdc

# Calculate ratio manually or use the --ratio flag
bash .cursor/skills/CRUX-Token-Count-Estimator/scripts/count-tokens.sh --ratio rules/my-rule.mdc rules/my-rule.crux.mdc
```
